...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (updateResult.count === 0) {
      ...
      return NextResponse.json({ message: "Using cached state", match: cachedMatch });
    }
    ...
    if (newSolves.length === 0) {
      ...
      return NextResponse.json({ updated: false, match: updatedMatch }, { status: 200 });
    }
    ...
    return NextResponse.json({ updated: true, match: updatedMatch }, { status: 200 });
  } catch (err) {
