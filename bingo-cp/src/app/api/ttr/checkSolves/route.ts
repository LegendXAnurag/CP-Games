...
export async function POST(req: NextRequest) {
    ...
    const { matchId, token } = (await req.json());

    if (!matchId || !token) {
        return NextResponse.json({ error: 'Missing matchId or token' }, { status: 400 });
    }
    ...
    try {
        ...
        if (now.getTime() - lastPolled.getTime() <= cooldownSeconds * 1000) {
            ...
            return NextResponse.json({ queued: false, reason: 'cooldown' }, { status: 200 });
        }
        ...
        // Note: In App Router, fire-and-forget background work after returning
        // is not natively supported. The response is returned, but the background
        // work still runs in the same request lifecycle.
